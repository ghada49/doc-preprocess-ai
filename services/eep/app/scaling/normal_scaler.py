...
_RUNPOD_URL_ENV_NAMES = ("IEP0_URL", "IEP1A_URL", "IEP1B_URL")
    ...
    if runpod_api_key:
        try:
            if runpod_pod_mode == "existing":
                ...
                reusable_urls = _runpod_urls_from_started_eep_worker(ecs_client, cluster)
                if reusable_urls is not None:
                    iep0_url, iep1a_url, iep1b_url = reusable_urls
                    logger.info(
                        "normal_scaler: reusing RunPod URLs from in-progress eep-worker "
                        "startup instead of creating another RunPod triplet"
                    )
                else:
...
def _looks_like_runpod_proxy_url(url: str) -> bool:
    """Return True for the stable RunPod proxy URLs generated by this scaler."""
    if not url.startswith("https://"):
        return False
    host = url.removeprefix("https://").split("/", 1)[0]
    return host.endswith(".proxy.runpod.net") and "-" in host


def _runpod_urls_from_task_definition(task_definition: dict) -> tuple[str, str, str] | None:
    """Extract the RunPod URL triplet from an eep-worker task definition."""
    for container in task_definition.get("containerDefinitions") or []:
        ...
        urls = tuple(env_by_name.get(name, "").strip() for name in _RUNPOD_URL_ENV_NAMES)
        if all(_looks_like_runpod_proxy_url(url) for url in urls):
            return urls  # type: ignore[return-value]
    return None


def _runpod_urls_from_started_eep_worker(ecs_client, cluster: str) -> tuple[str, str, str] | None:
    """
    Reuse the RunPod URL triplet selected by an in-progress eep-worker startup.

    The short Redis lock only protects concurrent callers while AWS calls are in
    flight. ECS warm-up takes longer, so a later trigger may see services below
    the fully-active threshold. In that case we still want to heal/redeploy ECS,
    but we must not create another RunPod triplet if eep-worker is already
    desired, pending, or running with valid env-baked RunPod URLs.
    """
    try:
        ...
        logger.warning(
            "normal_scaler: could not inspect eep-worker for RunPod URL reuse: %s",
            exc,
        )
        return None
    ...
    service = next(
        (svc for svc in services if svc.get("serviceName") == "libraryai-eep-worker"),
        None,
    )
    if service is None:
        return None
    ...
    if desired < 1 and running < 1 and pending < 1:
        return None

    task_definition_arn = service.get("taskDefinition") or "libraryai-eep-worker"
    try:
        task_def_resp = ecs_client.describe_task_definition(taskDefinition=task_definition_arn)
    except Exception as exc:  # noqa: BLE001
        logger.warning(
            "normal_scaler: eep-worker is started but task definition %r could not "
            "be inspected for RunPod URL reuse: %s",
            task_definition_arn,
            exc,
        )
        return None

    urls = _runpod_urls_from_task_definition(task_def_resp.get("taskDefinition") or {})
    if urls is None:
        logger.warning(
            "normal_scaler: eep-worker is started but task definition %r has no "
            "complete RunPod URL triplet; creating a fresh triplet",
            task_definition_arn,
        )
        return None

    logger.info(
        "normal_scaler: eep-worker already started "
        "(desired=%d,running=%d,pending=%d,task_definition=%s); reusing RunPod URLs",
        desired,
        running,
        pending,
        task_definition_arn,
