...
from services.iep1a.app.inference import get_model_info, is_model_ready, reload_models
...
_RELOAD_CHANNEL = "libraryai:model_reload:iep1a"
...
def _start_reload_subscriber() -> None:
    ...
    def _subscriber() -> None:
        try:
            ...
            logger.info("iep1a: subscribed to %s", _RELOAD_CHANNEL)
            for message in pubsub.listen():
                if message.get("type") == "message":
                    version = (message.get("data") or b"").decode("utf-8", errors="replace")
                    logger.info("iep1a: model_reload signal received version=%r — clearing cache", version)
                    reload_models(version_tag=version or None)
        except Exception:
            logger.exception("iep1a: model_reload subscriber thread failed")

    t = threading.Thread(target=_subscriber, daemon=True, name="iep1a-reload-subscriber")
    t.start()
...
@app.get(
    "/model-info",
    summary="Current model-loading state for this iep1a instance",
)
def model_info() -> dict:
    """
    ...
    ``version_tag`` is always ``null`` — iep1a loads weights from local .pt
