"""
services/iep1a/app/geometry.py
-------------------------------
IEP1A POST /v1/geometry router.
...
from services.iep1a.app.inference import InferenceError, run_mock_inference
...
@router.post(
    ...
    summary="Run IEP1A geometry inference on a proxy image",
)
def geometry(body: GeometryRequest) -> GeometryResponse | JSONResponse:
    """
    Run IEP1A (YOLOv8-seg) geometry inference on the provided proxy image.
    ...
    try:
        ...
        logger.warning(
            "IEP1A inference error: %s (fallback_action=%s)",
            err.error_code,
            err.fallback_action,
        )
