"""
services/retraining_worker/app/reconcile.py
-------------------------------------------
Retraining recovery reconciliation — runs as a second background loop inside
the retraining-worker process.
...
Recovery logic per pass:
  ...
  2. Orphaned triggers: find retraining_triggers with status='processing'
     whose linked retraining_job is either missing or has already failed →
     mark status='failed'.
...
async def run_reconciliation_loop(
    ...
    cfg = config or ReconcileConfig()
    logger.info(
        "retraining_worker: reconciliation loop started (interval=%.0fs timeout=%dmin)",
        interval_seconds,
    ...
    while True:
        ...
        try:
            ...
            if result.recovered_jobs or result.recovered_triggers:
                logger.info(
                    "retraining_worker: reconcile pass — recovered_jobs=%d recovered_triggers=%d",
                    result.recovered_jobs,
                    result.recovered_triggers,
                )
        except Exception:
            logger.exception("retraining_worker: error during reconciliation cycle")
        finally:
            db.close()
