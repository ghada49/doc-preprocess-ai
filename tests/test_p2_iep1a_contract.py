"""
tests/test_p2_iep1a_contract.py
--------------------------------
Packet 2.1 contract tests for IEP1A POST /v1/geometry.
...
Covers:
  ...
  - is_model_ready() returns False when IEP1A_MOCK_NOT_READY=true

Tests import the geometry router into a minimal FastAPI app to avoid pulling in
prometheus_client (which configure_observability requires but is not installed
in the test environment). Readiness-check logic is tested directly on
is_model_ready() rather than via HTTP.
"""
...
import services.iep1a.app.inference as inference_mod
from services.iep1a.app.geometry import router
...
def _clean_mock_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for key in (
        ...
        "IEP1A_MOCK_FAIL_ACTION",
        "IEP1A_MOCK_PAGE_COUNT",
        "IEP1A_MOCK_CONFIDENCE",
        "IEP1A_MOCK_TTA_PASSES",
        "IEP1A_MOCK_TTA_AGREEMENT_RATE",
        "IEP1A_MOCK_TTA_VARIANCE",
        "IEP1A_MOCK_NOT_READY",
    ):
...
class TestReadinessLogic:
    ...
    def test_model_not_ready_when_env_set(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("IEP1A_MOCK_NOT_READY", "true")
...
class TestTwoPageSplit:
    ...
    def resp(
        ...
        monkeypatch.setenv("IEP1A_MOCK_PAGE_COUNT", "2")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200, r.text
        return r.json()  # type: ignore[no-any-return]
...
class TestConfigurableConfidence:
    def test_low_confidence_reflected(
        ...
        monkeypatch.setenv("IEP1A_MOCK_CONFIDENCE", "0.30")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200
    ...
    def test_high_confidence_reflected(
        ...
        monkeypatch.setenv("IEP1A_MOCK_CONFIDENCE", "0.99")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200
    ...
    def test_per_region_confidence_matches_global(
        ...
        monkeypatch.setenv("IEP1A_MOCK_CONFIDENCE", "0.75")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200
...
class TestConfigurableTtaPasses:
    def test_tta_passes_reflected(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_PASSES", "3")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200
...
class TestFailureSimulation:
    def test_failure_returns_non_200(
        ...
        monkeypatch.setenv("IEP1A_MOCK_FAIL", "true")
        assert client.post("/v1/geometry", json=VALID_BODY).status_code != 200
    ...
    def test_escalate_review_returns_422(
        ...
        monkeypatch.setenv("IEP1A_MOCK_FAIL", "true")
        monkeypatch.setenv("IEP1A_MOCK_FAIL_ACTION", "ESCALATE_REVIEW")
    ...
    def test_retry_returns_503(self, client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("IEP1A_MOCK_FAIL", "true")
        monkeypatch.setenv("IEP1A_MOCK_FAIL_ACTION", "RETRY")
    ...
    def test_default_error_code_is_geometry_failed(
        ...
        monkeypatch.setenv("IEP1A_MOCK_FAIL", "true")
        monkeypatch.delenv("IEP1A_MOCK_FAIL_CODE", raising=False)
        assert (
    ...
    def test_custom_error_code_invalid_image_reflected(
        ...
        monkeypatch.setenv("IEP1A_MOCK_FAIL", "true")
        monkeypatch.setenv("IEP1A_MOCK_FAIL_CODE", "INVALID_IMAGE")
...
# ---------------------------------------------------------------------------
# Packet 2.2 — TTA agreement rate and variance
# ---------------------------------------------------------------------------
...
class TestTTAAgreementRate:
    ...
    def test_low_agreement_rate_reflected(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_AGREEMENT_RATE", "0.6")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200
    ...
    def test_high_agreement_rate_reflected(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_AGREEMENT_RATE", "0.9")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200
    ...
    def test_agreement_rate_in_range(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_AGREEMENT_RATE", "0.75")
        r = client.post("/v1/geometry", json=VALID_BODY)
...
class TestTTAVariance:
    ...
    def test_high_variance_reflected(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_VARIANCE", "0.5")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200
    ...
    def test_variance_non_negative(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_VARIANCE", "0.05")
        r = client.post("/v1/geometry", json=VALID_BODY)
...
class TestUncertaintyFlags:
    ...
    def test_low_agreement_adds_flag(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_AGREEMENT_RATE", "0.5")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert r.status_code == 200
    ...
    def test_agreement_at_threshold_no_flag(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_AGREEMENT_RATE", "0.80")
        r = client.post("/v1/geometry", json=VALID_BODY)
    ...
    def test_variance_at_threshold_no_flag(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_VARIANCE", "0.10")
        r = client.post("/v1/geometry", json=VALID_BODY)
    ...
    def test_high_agreement_no_low_flag(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_AGREEMENT_RATE", "1.0")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert "low_structural_agreement" not in r.json()["uncertainty_flags"]
    ...
    def test_low_variance_no_high_flag(
        ...
        monkeypatch.setenv("IEP1A_MOCK_TTA_VARIANCE", "0.001")
        r = client.post("/v1/geometry", json=VALID_BODY)
        assert "high_prediction_variance" not in r.json()["uncertainty_flags"]
